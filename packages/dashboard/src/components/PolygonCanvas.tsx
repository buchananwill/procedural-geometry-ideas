...
export default function PolygonCanvas({
  ...
  // Touch: single-finger tap-or-pan + pinch-to-zoom
  const PAN_THRESHOLD = 8; // px of movement before a touch becomes a pan
  const lastTouchDist = useRef(0);
  const lastTouchCenter = useRef({ x: 0, y: 0 });
  const isTouchPanning = useRef(false);
  const touchPanStart = useRef({ x: 0, y: 0 });
  const touchStartPos = useRef({ x: 0, y: 0 }); // raw screen position at touchstart
  ...
  const handleStageClick = useCallback(
    (e: KonvaEventObject<MouseEvent>) => {
      // Only respond to left-click (button 0) — ignore middle-click, right-click, etc.
  ...
  const handleTouchStart = useCallback(
    (e: KonvaEventObject<TouchEvent>) => {
      ...
      if (touches.length === 2) {
        // Pinch start — record initial distance and center
        e.evt.preventDefault();
        ...
        // Record start position; don't commit to panning yet (might be a tap)
        const isOnVertex = e.target.getClassName() === 'Circle' && e.target.draggable();
        touchOnVertex.current = isOnVertex;
        if (isOnVertex) return; // let Konva handle vertex drag
        isTouchPanning.current = false;
  ...
  const handleTouchMove = useCallback(
    (e: KonvaEventObject<TouchEvent>) => {
      ...
      if (touches.length === 2) {
        ...
        // Single-finger: check if movement exceeds threshold before panning
  ...
  /** If the finger never moved past the threshold, treat it as a tap → add vertex */
  const handleTouchEnd = useCallback(
    (e: KonvaEventObject<TouchEvent>) => {
      ...
      // If it was a vertex drag or a pan gesture, nothing to do
      ...
      // Treat as a tap — try to add a vertex on the nearest edge
      ...
      for (let i = 0; i < vertices.length; i++) {
        const a = vertices[i];
        const b = vertices[(i + 1) % vertices.length];
        const result = distanceToSegment(logicalX, logicalY, a.x, a.y, b.x, b.y);
        if (result.distance < bestDist) {
          bestDist = result.distance;
          bestIndex = i + 1;
          bestPoint = result.point;
    ...
    [vertices, addVertex, setSelectedVertex, stageScale, stagePosition, invScale],
  );
