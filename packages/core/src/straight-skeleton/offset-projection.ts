...
function requireProjectableResult(result: SkeletonSolveResult, caller: string): StraightSkeletonSolverContext {
    if (!result.complete) {
        throw new Error(
...
/** Drop consecutive coincident vertices, including across the wrap from last back to first. */
function dropAdjacentDuplicates(ring: Vector2[]): Vector2[] {
    const deduped: Vector2[] = [];
    for (const vertex of ring) {
        if (deduped.length === 0 || !vectorsAreEqual(deduped[deduped.length - 1], vertex)) {
            deduped.push(vertex);
        }
    }
    while (deduped.length > 1 && vectorsAreEqual(deduped[0], deduped[deduped.length - 1])) {
        deduped.pop();
    }
    return deduped;
}
...
function assembleRings(segments: WavefrontSegment[]): Vector2[][] {
    ...
    for (let seed = 0; seed < segments.length; seed++) {
        ...
        for (;;) {
            visited.add(cursor);
            ring.push(segments[cursor].startPosition);
        ...
        const deduped = dropAdjacentDuplicates(ring);
        if (deduped.length >= 3) {
            rings.push(deduped);
        }
    }
...
export function computeOffsetRings(result: SkeletonSolveResult, offset: number): Vector2[][] {
    const context = requireProjectableResult(result, 'computeOffsetRings');

    if (!(offset >= 0)) {
        throw new Error(`computeOffsetRings requires a non-negative offset, received ${offset}.`);
    }
