...
describe('compile() -- opcode table', () => {
    ...
    it('assigns f=5 as the sixth keyword', () => {
        const cs = compile(SQUARE_CONFIG);
        expect(cs.reverseTable[5]).toBe('f');
    });

    it('assigns letter opcodes starting at NUM_KEYWORDS (6)', () => {
        const cs = compile(SQUARE_CONFIG);
        expect(cs.opcodeTable.get('X')).toBe(6);
    });

    it('assigns multiple letters in sorted key order', () => {
        const cs = compile(BRANCH_CONFIG);
        expect(cs.opcodeTable.get('stem')).toBe(6);
        expect(cs.opcodeTable.get('tip')).toBe(7);
    });

    it('numKeywords is always 6', () => {
...
describe('compile() -- productions array', () => {
    it('keyword productions are identity: productions[k] = [k] for k in 0..5', () => {
        const cs = compile(SQUARE_CONFIG);
        for (let k = 0; k < 6; k++) {
    ...
    it('compiles X -> F + X correctly', () => {
        const cs = compile(SQUARE_CONFIG);
        expect(cs.productions[6]).toEqual([0, 1, 6]);
    });

    it('unspecified letter production defaults to identity', () => {
        const cs = compile(BRANCH_CONFIG);
        expect(cs.productions[7]).toEqual([7]);
    });

    it('compiles stem -> stem tip correctly', () => {
        const cs = compile(BRANCH_CONFIG);
        expect(cs.productions[6]).toEqual([6, 7]);
    });
});
...
describe('compile() -- definitions array', () => {
    ...
    it('compiles X definition (F) correctly', () => {
        const cs = compile(SQUARE_CONFIG);
        expect(cs.definitions[6]).toEqual([0]);
    });

    it('compiles tip definition ([+F][-F]) correctly', () => {
        const cs = compile(BRANCH_CONFIG);
        expect(cs.definitions[7]).toEqual([3, 1, 0, 4, 3, 2, 0, 4]);
    });
});
